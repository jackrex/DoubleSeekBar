...
public class DoubleSeekView extends View {
	...
	public void setmMiddleLength(int mMiddleLength) {
		this.mMiddleLength = mMiddleLength;
		invalidate();
	}
	...
	public DoubleSeekView(Context context, AttributeSet attrs, int defStyle) {
		...
		mScollBarHeight = array.getLayoutDimension(
				R.styleable.SeekBarPressure_height, "layout_height");
		array.recycle();
		...
		total = formatInt((mScollBarWidth - mThumbWidth / 2)
				* ((float) 103 / 100));
		aftOil = (int) (formatFloat((float) (mThumbWidth + mMiddleLength)
				/ total) * 100);
	...
	public int getAreaFlag(MotionEvent e) {
		...
		if (e.getY() >= 0 && e.getY() <= mThumbHeight
				...
				&& e.getX() < mOffsetLow + mMiddleLength) {
